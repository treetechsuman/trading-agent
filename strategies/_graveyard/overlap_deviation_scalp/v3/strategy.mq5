//+------------------------------------------------------------------+
//| overlap_deviation_scalp v3 - MOMENTUM CONTINUATION off deviation   |
//| from a moving average, M5 bars, London/New York overlap, scalping  |
//| only.                                                                |
//|                                                                      |
//| v1 (M1 fade) and v2 (M5 fade) both showed real in-sample signal      |
//| after tuning (win rate consistently near/above 50%, PF pushed       |
//| above 1.0 with healthy sample sizes) but neither survived           |
//| out-of-sample -- 10 combinations tested across mechanisms, pairs,   |
//| and tuning dimensions, all OOS failures. Every test so far bet on   |
//| MEAN REVERSION (fade the deviation). This is the one directional    |
//| hypothesis not yet tested with the full tuning methodology (the     |
//| original overlap_momentum_scalp v1 tested continuation, but only    |
//| once, unswept, on noisy M1 single-bar signals, before the M5-       |
//| smoothing and hold-time-tuning lessons were learned).                |
//|                                                                      |
//| v3 flips the direction: bet WITH a large deviation from the mean     |
//| (momentum/trend continuation) instead of against it, using the       |
//| same volatility-adaptive M5 signal and full tuning process.          |
//+------------------------------------------------------------------+
...
input double MinDeviationVsAvgMultiplier = 3.0;  // start from v2's locked value
...
input int    MagicNumber       = 20260062;
...
void CheckMonthlyHaltResume(datetime now)
{
   ...
   if(s.year > haltSetYear || (s.year == haltSetYear && s.mon > haltSetMonth))
   {
      ...
      consecutiveLosses = 0;
      Print("overlap_deviation_scalp v3: new month -- consecutive-loss pause lifted.");
   }
}
...
void UpdateDrawdownHalt()
{
   ...
   if(balancePeak > 0 && (balancePeak - balanceNow) >= balancePeak * InpMaxDrawdownStopPercent / 100.0)
   {
      if(!drawdownHaltActive)
         Alert("overlap_deviation_scalp v3: drawdown stop hit (", DoubleToString(InpMaxDrawdownStopPercent, 1),
...
void ProcessClosedBar(datetime now)
{
   ...
   if(deviation > 0)
   {
      // Price above the mean -- CONTINUATION: bet it keeps extending, buy
   ...
   {
      // Price below the mean -- CONTINUATION: bet it keeps extending, sell
...
int OnInit()
{
   ...
   {
      Alert("overlap_deviation_scalp v3: refusing to start on a REAL account -- set InpAllowLiveAccount=true to confirm.");
      Print("overlap_deviation_scalp v3: refusing to start on a REAL account -- set InpAllowLiveAccount=true to confirm.");
      return INIT_FAILED;
   }
...
void OnTradeTransaction(const MqlTradeTransaction &trans, const MqlTradeRequest &request, const MqlTradeResult &result)
{
   ...
   if(profit < 0)
   {
      ...
      if(consecutiveLosses >= InpMaxConsecutiveLosses && !monthlyHaltActive)
      {
         ...
         haltSetYear = s.year;
         haltSetMonth = s.mon;
         Print("overlap_deviation_scalp v3: ", InpMaxConsecutiveLosses,
