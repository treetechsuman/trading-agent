//+------------------------------------------------------------------+
//| overlap_deviation_scalp v5 - v4's fade mechanism (M5 deviation from |
//| a moving average, full London session) plus a VOLUME CONFIRMATION   |
//| filter, scalping only.                                              |
//|                                                                      |
//| v4 reached the most statistically confident result of this whole    |
//| investigation -- in-sample PF 1.09 on 1230 trades, every year        |
//| individually profitable, win rate a remarkably consistent           |
//| 50.6-51.1% across 2021/2022/2023 -- and still failed out-of-sample   |
//| on an equally large sample (PF 0.81, 727 trades, win rate dropped    |
//| to 46.35%). 12 combinations tested total (direction, bar             |
//| granularity, symbol, selectivity, hold time, session width), all     |
//| failed OOS. Every signal used so far has been pure price action.     |
//|                                                                      |
//| v5 adds a dimension not yet tried: MT5 supplies tick volume per      |
//| bar. Requiring the triggering bar's tick volume to be above its own  |
//| recent rolling average (same adaptive-threshold technique as the     |
//| price deviation filter) tests whether genuine participation/        |
//| liquidity behind a deviation, not just its price size, is what       |
//| distinguishes a real reversion setup from noise or a thin-liquidity  |
//| spike.                                                                |
//+------------------------------------------------------------------+
...
input int    SessionStartHour     = 10;   // 08:00 London == 10:00 server, year-round (full London session start)
input int    SessionEndHour       = 18;   // 16:00 London == 18:00 server (through the NY overlap's end)

// -- Deviation-from-mean detection (volatility-adaptive, M5 bars) -----------
input int    SMAPeriod                  = 10;  // M5 bars (50 min lookback)
input double MinDeviationVsAvgMultiplier = 3.0;  // starting from v2's locked value
input double MinDeviationPipsFloor      = 4.0;
input int    DeviationHistoryWindow     = 15;  // M5 bars (75 min)
input int    MinHistoryToAdapt          = 6;

// -- Volume confirmation (volatility-adaptive, same technique) --------------
input double MinVolumeVsAvgMultiplier   = 1.2;  // triggering bar's tick volume must be >= this x its own recent rolling average
input int    VolumeHistoryWindow        = 15;   // M5 bars
input int    MinVolumeHistoryToAdapt    = 6;
...
input int    MagicNumber       = 20260064;
...
double AverageVolumeHistory()
{
   double sum = 0;
   for(int i = 0; i < volumeHistoryCount; i++)
      sum += volumeHistory[i];
   return sum / volumeHistoryCount;
}

void PushVolumeHistory(double vol)
{
   volumeHistory[volumeHistoryNext] = vol;
   volumeHistoryNext = (volumeHistoryNext + 1) % VolumeHistoryWindow;
   if(volumeHistoryCount < VolumeHistoryWindow)
      volumeHistoryCount++;
}
...
void ProcessClosedBar(datetime now)
{
   ...
   double barVolume = (double)iVolume(_Symbol, PERIOD_M5, 1);

   // Feed both rolling histories regardless of pass/fail, so neither
   // baseline is distorted by excluding the events it exists to detect.
   PushDeviationHistory(devPips);
   PushVolumeHistory(barVolume);
   ...
   double volThreshold;
   if(volumeHistoryCount < MinVolumeHistoryToAdapt)
      volThreshold = 0; // not enough history yet -- don't block on volume until we have a baseline
   else
      volThreshold = MinVolumeVsAvgMultiplier * AverageVolumeHistory();

   if(barVolume < volThreshold)
      return; // deviation wasn't backed by above-average participation -- skip
...
int OnInit()
{
   ...
   ArrayResize(volumeHistory, VolumeHistoryWindow);
