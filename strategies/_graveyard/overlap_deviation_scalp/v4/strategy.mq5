//+------------------------------------------------------------------+
//| overlap_deviation_scalp v4 - v2's fade mechanism (M5 deviation from |
//| a moving average), same directional logic, but widened to the       |
//| FULL London session instead of just the London/NY overlap.          |
//|                                                                      |
//| v2 (M5 fade, overlap only) reached in-sample PF 1.15 (500 trades,    |
//| healthiest result of this whole investigation) but failed            |
//| out-of-sample (PF 0.84). v3 confirmed continuation is the wrong      |
//| direction (PF 0.71). This tests the other session option offered    |
//| alongside the overlap: the full London session (08:00-16:00         |
//| London = 10:00-18:00 server), in case the overlap's narrower window  |
//| was cutting out real opportunities rather than just noise.           |
//+------------------------------------------------------------------+
...
input int    MagicNumber       = 20260063;
