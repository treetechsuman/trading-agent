//+------------------------------------------------------------------+
//| nfp_fade v3 - fade the initial spike around the US Non-Farm       |
...
//|                                                                      |
//| SINGLE CHANGE FROM v2 (per v3/spec.json / v2/review.md): a          |
//| data-integrity audit of the hardcoded 108-entry NFP release-date    |
//| table, NOT a logic/mechanism change. No parameter, threshold,       |
//| SL/TP, hold time, sizing, or safety rule differs from v2.           |
//|                                                                      |
//| ARCHITECTURE NOTE FOR RESEARCHER (important correction to the v3    |
//| task brief's premise): v2/review.md diagnosed the two dropped       |
//| dates (2022.02.04, 2022.04.01) as evidence of drift across "three   |
//| independently-typed per-symbol tables", one per EA instance. On     |
//| inspecting the actual v1 and v2 source trees, that premise does not |
//| match the real codebase: nfp_fade has ALWAYS been exactly ONE       |
//| strategy.mq5 file (see v1/strategy.mq5, v2/strategy.mq5), written   |
//| generically against _Symbol and deployed UNMODIFIED to all three    |
//| chart instances (EURUSD.r / GBPUSD.r / USDJPY) -- there has never   |
//| been a second, independently-typed copy of NfpReleaseTable for it   |
//| to drift from. Confirmed by direct inspection: both "dropped"       |
//| dates were already present in v2's single table before this        |
//| version existed. The GBP/JPY trade-count differences v2/review.md   |
//| traced per-pair almost certainly came from this EA's own SIGNAL     |
//| FILTERS (spike-vs-threshold, spread, price-data availability) —     |
//| which legitimately differ per instrument on the same calendar date  |
//| — not from the calendar table itself, since only one table exists.  |
//| Flagging this back to Researcher per EA Coder's role (note                |
//| discrepancies, don't silently "fix" a narrative mismatch) rather     |
//| than fabricating three separate per-symbol files to match a         |
//| premise that doesn't reflect this project's actual convention here  |
//| -- kept the existing single-file convention, which is structurally  |
//| safer against exactly this class of bug (there is no second copy    |
//| to diverge). See the audit block directly above NfpReleaseTable     |
//| below for the full entry-by-entry verification performed for v3.    |
//|                                                                      |
...
//|                                                                      |
//| ENTRY TIMING -- unchanged from v1/v2, the highest-risk part of      |
//| this EA to get right: 8:30am US Eastern must be converted to this   |
...
//| v1/v2 (already spot-checked against real broker tick timestamps by  |
//| Backtester and confirmed correct to the minute). Only the           |
//| NfpReleaseTable content/documentation changed in v3.                |
//+------------------------------------------------------------------+
...
//| release = the Friday 20 calendar days after the Saturday closing   |
//| the reference week (the week containing the 12th of the REPORTED   |
//| month, i.e. one calendar month before the release month) -- BLS's  |
//| own documented scheduling rule.                                    |
//|                                                                      |
//| v3 FULL AUDIT (2026-09-23, EA Coder, in response to v2/review.md's |
//| finding that 2022.02.04 and 2022.04.01 appeared dropped from some   |
//| per-pair backtest journals): re-derived the base rule above         |
//| programmatically and checked ALL 108 entries against it (not a      |
//| sample). Result: 104/108 entries match the base rule exactly,       |
//| including both dates named in v3/spec.json as needing restoration   |
//| -- both were already present in this table. The remaining 4/108     |
//| are EXACTLY the 4 exceptions already documented below (2020.01.10,  |
//| 2021.01.08, 2025.01.10, 2025.07.03) -- zero additional, previously  |
//| undocumented deviations found anywhere in the table. This is a      |
//| stronger check than the original 10-date spot-check: every single   |
//| entry was verified against the rule, not just a sample. See the     |
//| ARCHITECTURE NOTE at the top of this file for why "restoring"       |
//| dropped dates and a "three-way diff" reduce to this single audit —  |
//| there is, and has only ever been, one table in one file.            |
//|                                                                      |
//| v3 OUT-OF-SAMPLE (2023-2025) SPOT-CHECK, per spec.json's explicit   |
//| requirement (>=10 dates, confidence tier per date; no live internet |
//| access in this coding session, so checked against recalled          |
//| real-world news/market events tied to the specific release date,    |
//| not just the formula):                                              |
//|   HIGH (specific recalled real-world event on this exact date):     |
//|     2023.03.10 (SVB collapse, same day), 2023.04.07 (Good Friday    |
//|     release, stock market closed that day), 2023.09.01 (pre-Labor   |
//|     Day Friday), 2024.07.05 (day after Jul 4 holiday), 2024.08.02    |
//|     (weak report, "Sahm rule" recession-indicator selloff),          |
//|     2025.01.10 and 2025.07.03 (both already-documented exceptions,   |
//|     independently recalled, not just formula output).                |
//|   MODERATE (matches the documented rule/typical pattern, not tied    |
//|     to a specific recalled news event -- recommend an external BLS   |
//|     calendar check if convenient before/alongside OOS validation):   |
//|     2023.11.03, 2024.01.05, 2024.11.01, 2025.02.07, 2025.09.05.       |
//|   FLAG FOR EXTERNAL CHECK (rule-consistent, but a real-world event   |
//|     could plausibly have disrupted BLS's normal schedule in a way    |
//|     not reliably in this session's recall): 2025.10.03 -- the Oct    |
//|     2025 US federal government shutdown began 2025.10.01, and a      |
//|     shutdown can delay a BLS release; this table's Oct 2025 entry    |
//|     is the formula's un-shifted date and has NOT been confirmed      |
//|     against a live source. Recommend Researcher/Backtester verify    |
//|     this one specifically before relying on it during OOS            |
//|     validation -- if it's wrong, only that single month's trade for  |
//|     each pair is affected.                                          |
//|                                                                      |
//| Two confirmed manual exceptions applied (unchanged from v2):         |
//| 2020.01.10 (not the rule's 2020.01.03) and 2021.01.08 (not the       |
//| rule's 2021.01.01, itself New Year's Day) -- both already flagged    |
//| in v1/review.md. A third, same-pattern January exception             |
//| (2025.01.10, not the rule's 2025.01.03) and one Independence-Day     |
//| exception (2025.07.03, a Thursday, not the rule's 2025.07.04) are    |
//| also applied. See dates.md / EA Coder's handoff for confidence       |
//| caveats.                                                             |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Calendar helpers (unchanged from v1/v2 -- still needed for the     |
//| DST conversion below, which is independent of the NFP day-         |
...
//| Unchanged from v1/v2 -- computed per calendar year from first      |
...
//| is, so it is reused verbatim against the table's dates.             |
//+------------------------------------------------------------------+
