//+------------------------------------------------------------------+
//| month_end_fix_reversal v2 - fade the WM/Reuters 4pm London FX fix  |
...
//| filter.                                                              |
//|                                                                      |
//| v2 (2026-09-23) is a SAFETY-ONLY patch on top of v1, requested      |
//| after Live Manager found v1 had no live-account guard at all        |
//| (unlike sibling gotobi/v1). Adds InpAllowLiveAccount + the matching |
//| OnInit() refusal check, mirroring gotobi/v1's exact pattern.        |
//| Zero entry/exit/sizing/threshold logic changed from v1 -- see       |
//| v2/spec.json and ea-coder/lessons.md for the diff-verification      |
//| note. All v1 backtest results (review.md, VERDICT.md) apply         |
//| unchanged to v2, since ACCOUNT_TRADE_MODE is never                  |
//| ACCOUNT_TRADE_MODE_REAL inside the Strategy Tester.                 |
//+------------------------------------------------------------------+
